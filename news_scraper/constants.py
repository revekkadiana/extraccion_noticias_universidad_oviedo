
INVALID_URL_WORDS = {
    'section', 'tag', 'template', 'category', 'author', 'page-sitemap',
    'categories', 'video', 'image', 'temas', 'live', 'microsite', 'focus', 'blog',
    'ocio', 'cine', 'board', 'character', 'galeria', 'categoria', 'ficha',
    'firmante', 'secciones', 'hemeroteca', 'landing', 'sorteo', 'elecciones',
    'autor', 'hilos', 'cartelera'
}

# Namespaces para el parsing de sitemaps
SITEMAP_NAMESPACES = {
    'ns': 'http://www.sitemaps.org/schemas/sitemap/0.9',  # Namespace por defecto
    'news': 'http://www.google.com/schemas/sitemap-news/0.9'  # Namespace de Google News
}
